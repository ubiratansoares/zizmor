...
pub(crate) struct Expansions<'doc> {
    ...
    indeterminate_inclusions: Option<SymbolicLocation<'doc>>,
...
impl<'doc> Expansions<'doc> {
    ...
    /// Exposes an iterator over the expansions already evaluated
    pub(crate) fn iter(&self) -> impl Iterator<Item = &Expansion<'doc>> {
        self.all_expansions.iter()
    }

    /// Exposed so findings can annotate with the related `SymbolicLocation`
    pub(crate) fn indeterminate_expansions(&self) -> &Option<SymbolicLocation<'doc>> {
        &self.indeterminate_expansions
    }

    /// Exposed so findings can annotate with the related `SymbolicLocation`
    pub(crate) fn indeterminate_inclusions(&self) -> &Option<SymbolicLocation<'doc>> {
        &self.indeterminate_inclusions
    }
    ...
    pub(crate) fn has_indeterminations(&self) -> bool {
        self.indeterminate_expansions.is_some()
            || self.indeterminate_inclusions.is_some()
            || self.indeterminate_exclusions.is_some()
    }
    ...
    fn expand_values(matrix: &LoE<job::Matrix>, location: SymbolicLocation<'doc>) -> Self {
        match matrix {
            LoE::Expr(_) => Self {
                all_expansions: vec![],
                indeterminate_expansions: Some(location.clone()),
                indeterminate_inclusions: None,
            ...
            LoE::Literal(inner) => {
                ...
                Self {
                    ...
                    indeterminate_expansions: None,
                    indeterminate_inclusions: maybe_misses_inclusions,
  ...
  test:
    ...
    steps:
      - run: true
        ...
        assert!(expansions.indeterminate_inclusions.is_none());
  ...
  indirect-matrix:
    ...
    steps:
      - name: Noop
        ...
        assert!(expansions.indeterminate_expansions.is_none());
        assert!(expansions.indeterminate_inclusions.is_some());
